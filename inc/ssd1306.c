...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
    ...
    if (c >= '0' && c <= '9')
    {
        index = (c - '0' + 1) * 8; // Índice para números (0-9)
    }
    else if (c >= 'A' && c <= 'Z')
    {
        index = (c - 'A' + 11) * 8; // Índice para letras maiúsculas (A-Z)
    }
    else if (c >= 'a' && c <= 'z')
    {
        index = (c - 'a' + 37) * 8; // **Ajuste do índice para minúsculas**
    }
    else
    {
        return; // Se não for suportado, não desenha nada
    }
...
void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y)
{
    while (*str)
    {
        ...
        x += 8; // Avança para o próximo caractere
